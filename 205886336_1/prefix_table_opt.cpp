...
#include <sstream> 
 ...
 string prefix_table_opt::transpose(string ip)        //return 32 bits of ip
{
  int x = 0;  //for subnet
  int p1 = ip.find(".");
...
void prefix_table_opt::read_line(string line)
   {
      ...
      //string ip_new = transpose(ip);
       char result;
        if(function == "ADD")
        {
          ...
          add(prefix_table_opt::root,ip,ip_new,result);
        }
        else if(function == "REMOVE")
        {
          ...
          remove(prefix_table_opt::root,ip,ip_new,result,0);
        }
        else if(function == "FIND")
        {
          string ip = line.substr(5,line.find(" ",5));
          string ip_new = transpose(ip);
          find(prefix_table_opt::root,ip,ip_new);
        }
  ...
  void prefix_table_opt::read_from_file(string file_name)
{
...
void prefix_table_opt::add(Node* root,string ip_src,string ip,char result)
{
  Node* p  = root;
  ...
  int bit = (ip[ip.length()-1]+1)%2;
  if(p->children[bit] && p->children[bit]->result==result){
  ...
  else { // אם האח של הקודקוד יש לו result שונה
    int bit = ip[ip.length()-1];
    p->children[bit] = new Node();
    p = p->children[bit];
    p->isEnd = true;
    p->result = result;
...
bool prefix_table_opt::isEmpty(Node* root) 
{ 
    for (int i = 0; i < 2; i++) 
        if (root->children[i]) 
            return false; 
...
Node* prefix_table_opt::remove(Node* current,string ip_src ,string ip,char result, int depth)
{
     if(current == NULL)
        return NULL;
     if(depth == ip.length())
 ...
 void prefix_table_opt::find(Node* root,string ip_src,string ip)
{
  Node* p = root;
  for(int i=0;i<ip.length();i++)
   {
     int bit = ip[i];
      if(p->isEnd) {
        cout<<"Found"<<ip<<" "<<p->result<<" at the depth"<<i<<endl;
        return;
      }
      p = p->children[bit];
   }
   if(p != NULL && p->isEnd)
   {
     cout<<"Found"<<ip_src<<" "<<p->result<<" at the depth"<<ip.length()<<endl;
   }
}
